...
public class OknoKlient extends Application  {
    private static final String background = "file:D:\\Programy\\Java\\projekt_warcaby_v2\\src\\sample\\background.jpg";
    Group layout = new Group();
    PionekClass[][] PionekClass = new PionekClass[8][8];
    Board[][] board = new Board[8][8];
    ArrayList<Pozycje> lista = new ArrayList<>();
    Client klient = new Client("127.0.0.1", 5000);
    ...
    public static GRACZ gracz = GRACZ.PINK;

    @Override
    public void start(Stage primaryStage) throws IOException {

        primaryStage.setTitle("warcaby klient");
        ...
        layout.setOnMouseClicked(event -> {
            ...
            if (col >= 0 && col < 8 && row >= 0 && row < 8 && OknoKlient.state == STATE.MULTI) {
        ...
        MenuLayout.setOnMouseClicked(event -> {
            ...
            if(mx >= 300 && mx <= 500){
                ...
                if(my >= 381 && my <= 410){
                    OknoKlient.state = STATE.MULTI;
                    Scene scene = new Scene(layout, 800, 800);
                    primaryStage.setScene(scene);
                    try {
                        serverSendAndRead(0, 0, 0, 0, send);
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                }
        ...
        Image szczurek = new Image(background);
    ...
    void ilosc_klikow(Pozycje p) throws IOException {
        ...
        if (lista.size() == 1) {
            ...
            if (PionekClass[fromcol][fromrow].co == EMPTY) {
                ...
                lista.clear();
            } else if (PionekClass[fromcol][fromrow].co != ORANGE && OknoKlient.gracz == GRACZ.ORANGE) {
                System.out.println("nie tan Pionek");
                lista.clear();
            } else if (PionekClass[fromcol][fromrow].co != PINK && OknoKlient.gracz == GRACZ.PINK) {
                System.out.println("nie tan Pionek");
                lista.clear();
            }
        }
    ...
    void serverSendAndRead(int fromcol, int fromrow, int tocol, int torow, boolean send) throws IOException {
        reader = new BufferedReader(new InputStreamReader(klient.socket.getInputStream()));
        String wspolrzedne;
        PrintWriter pr = new PrintWriter(klient.socket.getOutputStream());

        if (send){
            pr.println(fromcol + "" + fromrow + "" + tocol + "" + torow);
            pr.flush();
        }

        wspolrzedne = reader.readLine();
    ...
    void sprawdzenie(int fromcol, int fromrow, int tocol, int torow) throws IOException {
        ...
        if(OknoKlient.gracz == GRACZ.PINK && tocol == fromcol + 1 && torow == fromrow + 1){
            System.out.println("tak nie mozna");
        }

        else if(OknoKlient.gracz == GRACZ.ORANGE && tocol == fromcol - 1 && torow == fromrow - 1){
            System.out.println("tak nie mozna");
        }
        ...
        if(OknoKlient.state == STATE.SINGLE && OknoKlient.gracz == GRACZ.PINK && poprawnyRuch){
            OknoKlient.gracz = GRACZ.ORANGE;
            kolor_gracza = kolor_gracza + 1;
        }
        else if (OknoKlient.state == STATE.SINGLE && OknoKlient.gracz == GRACZ.ORANGE && poprawnyRuch){
            OknoKlient.gracz = GRACZ.PINK;
            kolor_gracza = kolor_gracza + 1;
        }

        if(OknoKlient.state == OknoKlient.STATE.MULTI && poprawnyRuch){
            send = true;
            serverSendAndRead(fromcol, fromrow, tocol, torow, send);
        }
